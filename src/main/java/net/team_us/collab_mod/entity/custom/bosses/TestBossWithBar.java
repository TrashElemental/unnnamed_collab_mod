...
import net.team_us.collab_mod.entity.custom.BossEntity;
...
public class TestBossWithBar extends BossEntity implements GeoAnimatable {
    public TestBossWithBar(EntityType<? extends PathfinderMob> type, Level level) {
        super(type, level, "test_boss_with_bar", BossEvent.BossBarColor.RED);
    }
    ...
    public void onSpawnNaturally() {
        ...
        if (!level().isClientSide) {
            System.out.println("TestBossWithBar: Boss spawned via natural means.");
        }
    ...
    public void onSpawnViaCommand() {
        ...
        if (!level().isClientSide) {
            System.out.println("TestBossWithBar: Boss spawned via unnatural means.");
        }
    ...
    protected void actuallyHurt(DamageSource pDamageSource, float pDamageAmount) {
        ...
        if (!level().isClientSide) {
            System.out.println("TestBossWithBar (actuallyHurt): Boss health: " + this.getHealth() + "/" + this.getMaxHealth());
    ...
    public void onDeath(Entity killer) {
        ...
        if (!level().isClientSide) {
            System.out.println("TestBossWithBar: Boss was slain by" + killer);
        }
    ...
    private PlayState predicate(AnimationState<TestBossWithBar> state) {
        if (state.isMoving()) {
